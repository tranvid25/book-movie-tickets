    ...
    const operations = <Fragment>
        {_.isEmpty(userLogin) ? <Fragment>
            ...
            <Popover placement="bottomRight" title={userLogin.taiKhoan} content={content} trigger="click">
                <Button className='items-center justify-center w-full h-full p-0 rounded-full bg-slate-300 d-flex' style={{ width: 40, height: 40 }}>
                    {profile?.avatar !== null ?
...
function Checkout(props) {
    ...
    const renderGhe = () => {
        if (!Array.isArray(danhSachGhe) || danhSachGhe.length === 0) {
            return <div className="mt-10 text-center text-gray-400">Không có ghế nào cho suất chiếu này.</div>;
        }
            ...
            <div className='grid grid-cols-12'>
                ...
                <div className='col-span-4'>
                    <Card className='m-2'>
                        ...
                        <div className='text-xl font-bold text-red-400'>{minutes}:{seconds}</div>
                    </Card>
                    {thongTinPhim && (
                        <Card className='m-2'>
                            <h3 className='text-xl font-bold'>{thongTinPhim.tenPhim}</h3>
                            {/* <p>{thongTinPhim.rapChieu && thongTinPhim.rapChieu[0]?.tenRap}</p> */}
                            {rapChieu?.map && rapChieu.map((rap, index) => <p key={index}>{rap.tenRap}</p>)}
                    ...
                    <Card className='m-2'>
                        <p>Tổng đơn hàng</p>
                        <h3 className='text-xl font-bold text-red-400'>{tongTien.toLocaleString()} đ</h3>
                    </Card>
                    <Card className='m-2'>
                ...
                <div className='col-6'>
                    <div className=''>
                        <Card className='w-full m-2 bg-indigo-400'>
                            ...
                            <p className='text-lg'>Suất <b>{donhang?.gioChieu ? donhang.gioChieu.substr(0, 5) : ''}</b> Ngày <b>{donhang?.ngayChieu ? dayjs(donhang.ngayChieu).format('DD-MM-YYYY') : ''}</b></p>
                            <p className='text-lg'><b>{donhang?.tenRap || ''}</b> Ghế <b>{donhang?.danhSachGhe || ''}</b></p>
                        </Card>
                    </div>
                    <Card className='w-full m-2 bg-orange-400'>
                        <p>Tổng đơn hàng</p>
                        <h3 className='text-xl font-bold '>{donhang?.tongTien ? donhang.tongTien.toLocaleString() : ''} đ</h3>
                    </Card>
                    ...
                    <p className='ml-5 text-gray-400'>(*) Quý khách vui lòng kiểm tra kỹ thông tin, đơn hàng sau khi đặt sẽ không được hủy hoặc hoàn lại.</p>

                </div>
                <div className='col-6'>
                    <div className='row'>
                        <div className='mx-auto col-6'>
                            <h1 className='mb-5 text-xl text-center'>THANH TOÁN BẰNG THẺ TÍN DỤNG</h1>
                            {!show ? <Form
                                name="basic"
                                onFinish={onFinish}
                                autoComplete="off"
            ...
            <section className="text-gray-600 body-font">
                <div className="container px-5 py-24 mx-auto">
                    <div className="flex flex-col w-full mb-20 text-center">
                        <h1 className="mb-4 text-2xl font-medium text-gray-900 sm:text-3xl title-font">Lịch Sử Đặt Vé</h1>
                        <p className="mx-auto text-base leading-relaxed lg:w-2/3">Xin cám ơn bạn đã ủng hộ dịch vụ của chúng tôi, chúc bạn có những trải nghiệm tuyệt vời</p>
                    </div>
                    <div className="container">
                        {arrDonHang.length < 1 || arrDonHang == undefined ? <p className='w-full text-xl text-center'>Chưa có đơn hàng nào</p> :
                            <div>
                                <div className='row'>
                                ...
                                <div className='row'>
                                    {currentArrDonHang.slice(0, -1)?.map((item, index) => {
                                        return <div className='mt-3 col-6 '>
                                            <Card
                                                hoverable
                                ...
                                <Pagination className='justify-center my-20 d-flex line-clamp-3' pageSize={postsPerPage} currentPage={currentPage} total={arrDonHang.length} onChange={(page) => { setCurrentPage(page) }} />
                            </div>
                        }
                    </div>
                </div>
